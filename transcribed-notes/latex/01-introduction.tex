...
\newcommand{\StudyHeader}{Lecture 1 Exam Study Notes}
...
If $(X,\norm{\cdot}_X)$ is a normed space, then
\[
d(x,y):=\norm{x-y}_X
\]
...
Let $(X,d)$ be a metric space and $A\subset X$ nonempty.
\begin{itemize}
\item A point $x$ lies in $\ol A$ if and only if there exists a sequence $(x_n)\subset A$ with $x_n\to x$.
\item A set $A$ is closed if and only if every convergent sequence in $A$ has its limit in $A$.
\end{itemize}
\end{thmbox}
...
Let $Y$ be a subspace of a Banach space $X$. Then $Y$ is complete in the inherited norm if and only if $Y$ is closed in $X$.
\end{thmbox}
...
Let $f:X\to Y$ be a map between topological spaces. Then $f$ is continuous if and only if for every open set $U\subset Y$, the preimage $f^{-1}(U)$ is open in $X$.
\end{thmbox}
