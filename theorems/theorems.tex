...
\textbf{Convention.} The inner product $(\cdot,\cdot)_H$ is linear in the first argument and conjugate-linear in the second (physics convention).
...
Now let $f\in H^*$. By Riesz there exists $g\in H$ such that $f(v)=(v,g)_H$ for all $v$. Since $A$ is surjective, there exists a unique $u\in H$ with $Au=g$. Then
\[
B(v,u)=(v,Au)_H=(v,g)_H=f(v)
\]
for all $v$. Finally,
\[
\norm{u}\le \norm{A^{-1}}\norm{g}\le \beta^{-1}\norm{f}.
