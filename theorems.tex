...
Let $Y$ be a subspace of a Banach space $X$. Then $Y$ is complete with the inherited norm if and only if $Y$ is closed in $X$.
\end{theoremstmt}
...
Let $f:X\to Y$ be a map between topological spaces. Then $f$ is continuous if and only if $f^{-1}(U)$ is open in $X$ for every open set $U\subset Y$.
\end{theoremstmt}
...
=\sup_{\norm{x}_X=1}\norm{Lx}_Y.
\]
Then this defines a norm on the space $\mathcal L(X,Y)$ of bounded linear maps from $X$ to $Y$.
\end{theoremstmt}
...
If $Y$ is Banach, then $\bigl(\mathcal L(X,Y),\norm{\cdot}_{\mathcal L(X,Y)}\bigr)$ is Banach. In particular, for every normed space $X$, its dual $X^*=\mathcal L(X,\K)$ is Banach.
\end{theoremstmt}
...
If $\dim X=n<\infty$ and $L:X\to Y$ is linear, then $\dim \operatorname{Ran}L\le n$.
\end{theoremstmt}
...
\]
Then $T$ is a linear bijection. By Theorem 4.2, both $T$ and $T^{-1}$ are bounded, so $T$ is a topological isomorphism.
...
If $\norm{\cdot}_1$ and $\norm{\cdot}_2$ are two norms on $X$, the identity map from $(X,\norm{\cdot}_1)$ to $(X,\norm{\cdot}_2)$ is linear, hence bounded by Theorem 4.2, and likewise in the opposite direction. Therefore the norms are equivalent.
\end{proof}
...
If $X$ is finite-dimensional and $Y\subsetneq X$ is a proper subspace, then $\dim Y<\dim X$.
\end{theoremstmt}
...
\begin{proof}
If $X$ is finite-dimensional, then by Theorem 4.3 it is isomorphic to $\K^N$, where closed and bounded sets are compact. Hence $\Ball$ is compact.
...
\]
defines a metric on $X$. If this metric is complete, then $X$ is a Frechet space.
\end{theoremstmt}
...
Let $X$ be a real vector space, let $p:X\to\R$ be sublinear, let $V\subset X$ be a subspace, and let $f:V\to\R$ be linear with $f(v)\le p(v)$ for all $v\in V$. Then there exists a linear $F:X\to\R$ such that $F|_V=f$ and $F(x)\le p(x)$ for all $x\in X$.
\end{theoremstmt}
...
\begin{theoremstmt}{6.1'}
Let $X$ be a complex vector space, let $p:X\to\R$ satisfy $p(x+y)\le p(x)+p(y)$ and $p(\lambda x)=\abs{\lambda}p(x)$, let $V\subset X$ be a subspace, and let $f:V\to\C$ be linear with $\abs{f(v)}\le p(v)$ for all $v\in V$. Then there exists a linear $F:X\to\C$ such that $F|_V=f$ and $\abs{F(x)}\le p(x)$ for all $x\in X$.
\end{theoremstmt}
...
f(v)\le \abs{f(v)}\le \norm{f}\norm{v}=p(v).
\]
By Theorem 6.1 or 6.1', $f$ extends to some linear $F:X\to\K$ with $\abs{F(x)}\le p(x)$ for all $x$. Hence $F$ is bounded and $\norm{F}\le \norm{f}$. Since $F$ extends $f$, the reverse inequality $\norm{f}\le \norm{F}$ is immediate. Therefore $\norm{F}=\norm{f}$.
\end{proof}
...
\le \norm{y+tx_0}.
\]
Hence $\norm{f}_{Y_0^*}\le 1$. Since $f(x_0)=\operatorname{dist}(x_0,Y)$, in fact $\norm{f}=1$. By Theorem 6.2, extend $f$ to $\phi\in X^*$ with the same norm. Then $\phi$ has the required properties.
\end{proof}

\begin{theoremstmt}{6.4}
For every nonzero $x\in X$ there exists $\phi\in X^*$ such that $\phi(x)\ne 0$. Consequently weak convergence is well-defined.
\end{theoremstmt}

\begin{proof}
Take $Y=\{0\}$ in Theorem 6.3. Then for $x\neq 0$ there exists $\phi\in X^*$ with $\norm{\phi}=1$ and $\phi(x)=\norm{x}\neq 0$.
...
Items 1 and 2 follow immediately from the definition and Theorem 6.4. Item 3 holds because all linear topologies on a finite-dimensional space compatible with the vector structure coincide. For item 4, if $(e_n)$ is orthonormal in a Hilbert space, then Bessel's inequality gives $(x,e_n)\to 0$ for every fixed $x$, hence $e_n\rightharpoonup 0$, while $\norm{e_n}=1$ for all $n$. Items 5 and 6 are lower semicontinuity of the norm. Item 7 follows from Banach-Steinhaus after applying functionals to the sequence.
\end{proof}

\begin{theoremstmt}{7.1}
\textbf{(Banach-Alaoglu)} Let $X$ be a normed space. The closed unit ball of $X^*$ is compact in the weak-* topology. In particular, if $X$ is separable, every bounded sequence in $X^*$ has a weak-* convergent subsequence.
\end{theoremstmt}
...
If $X$ is reflexive, then the closed unit ball of $X$ is weakly compact. Equivalently, every bounded sequence in $X$ has a weakly convergent subsequence.
\end{theoremstmt}
...
\begin{proof}[Proof sketch]
The implication 1 $\Rightarrow$ 2 is Theorem 7.2. The implication 2 $\Rightarrow$ 3 is the sequential part of Eberlein-Smulian. The implication 3 $\Rightarrow$ 1 is the deep converse: if every bounded sequence has a weakly convergent subsequence, then the unit ball is weakly sequentially compact, hence weakly compact, and James' theorem or the Eberlein-Smulian machinery yields reflexivity.
\end{proof}

\section{Three Fundamental Theorems Based on Baire's Theorem}

\begin{theoremstmt}{8.1}
\textbf{(Cantor intersection theorem)} Let $(X,d)$ be complete and let $F_1\supset F_2\supset \cdots$ be nonempty closed sets with $\operatorname{diam}(F_n)\to 0$. Then $\bigcap_{n=1}^{\infty}F_n$ consists of exactly one point.
\end{theoremstmt}
...
\textbf{(Baire category theorem)} A complete metric space cannot be a countable union of nowhere dense sets. Equivalently, a countable intersection of dense open sets is dense.
\end{theoremstmt}
...
\textbf{(Variant of Baire)} If a complete metric space is the union of countably many closed sets, then at least one of those closed sets has nonempty interior.
\end{theoremstmt}
...
\textbf{(Banach-Steinhaus, uniform boundedness principle)} Let $X$ be Banach, $Y$ normed, and $\mathcal F\subset\mathcal L(X,Y)$. If for every $x\in X$,
\[
\sup_{L\in\mathcal F}\norm{Lx}_Y<\infty,
\]
then
...
\textbf{(Open mapping theorem)} Let $X,Y$ be Banach spaces and let $L\in\mathcal L(X,Y)$ be surjective. Then $L$ maps open sets in $X$ onto open sets in $Y$.
\end{theoremstmt}
...
\textbf{(Bounded inverse theorem)} Let $X,Y$ be Banach spaces and let $L\in\mathcal L(X,Y)$ be bijective. Then $L^{-1}\in\mathcal L(Y,X)$.
\end{theoremstmt}
...
I:(X,\norm{\cdot}_1)\to (X,\norm{\cdot}_2)
\]
is bounded by assumption and bijective. By Theorem 8.8, its inverse is bounded. Hence there exists $C'>0$ such that
\[
...
\textbf{(Closed graph theorem)} Let $X,Y$ be Banach spaces and let $L:X\to Y$ be linear. If
\[
\operatorname{Graph}(L)=\{(x,Lx):x\in X\}
\]
is closed in $X\times Y$, then $L\in\mathcal L(X,Y)$.
\end{theoremstmt}
...
I:(X,\norm{\cdot}_G)\to (X,\norm{\cdot}_X)
\]
is continuous and bijective. By Theorem 8.8, its inverse is bounded. Hence there exists $C>0$ such that
\[
\norm{x}_G\le C\norm{x}_X.
\]
...
If $K_n\in\mathcal L(X,Y)$ are compact and $K_n\to K$ in operator norm, then $K$ is compact.
\end{theoremstmt}
...
If $K\in\mathcal L(X,Y)$ is compact and $x_n\rightharpoonup x$ weakly in $X$, then $Kx_n\to Kx$ strongly in $Y$.
\end{theoremstmt}
...
\textbf{(Schauder theorem)} A bounded linear operator $K:X\to Y$ is compact if and only if its adjoint $K^*:Y^*\to X^*$ is compact.
\end{theoremstmt}
...
\qquad (f\in C([a,b])).
\]
Then $K:C([a,b])\to C([a,b])$ is compact.
\end{theoremstmt}
...
\textbf{(Cauchy-Schwarz inequality)} For all $x,y\in H$,
\[
\abs{(x,y)_H}\le \norm{x}_H\norm{y}_H.
\]
\end{theoremstmt}
...
\textbf{(Parallelogram rule)} For all $x,y\in H$,
\[
\norm{x+y}_H^2+\norm{x-y}_H^2=2\norm{x}_H^2+2\norm{y}_H^2.
\]
\end{theoremstmt}
...
\]
Hence $\sum_{k=1}^n (x,e_k)_He_k$ is the best approximation to $x$ from $\operatorname{span}\{e_1,\dots,e_n\}$.
\end{theoremstmt}
...
\textbf{(Bessel inequality and Parseval identity)} If $(e_k)_{k\ge 1}$ is orthonormal in $H$, then
\[
...
\begin{proof}
Apply Theorem 10.4 with $c_k=(x,e_k)_H$ to the first $n$ basis vectors:
\[
...
\]
is an isometry onto $\ol{\operatorname{span}}\{e_k:k\in\N\}$.
\end{theoremstmt}
...
\begin{proof}
Items 1 and 3 are the same statement. If the span is dense and $x$ is orthogonal to every $e_k$, then $x$ is orthogonal to a dense subspace, hence to all of $H$, so $x=0$; thus 3 implies 2. If 2 holds and $M$ is the closed span, then any $x\in M^\perp$ is orthogonal to every $e_k$, hence $x=0$; therefore $M^\perp=\{0\}$ and $M=H$, so 2 implies 3. Equivalence with 4 follows from Theorem 10.5.
\end{proof}

\begin{theoremstmt}{10.8}
Every separable Hilbert space has a complete orthonormal system.
\end{theoremstmt}
...
\begin{proof}
By Theorem 10.8, choose a complete orthonormal system $(e_k)$. Define
\[
T:H\to \ell^2,
\qquad
Tx=((x,e_k)_H)_{k\ge 1}.
\]
By Bessel, $Tx\in\ell^2$. By Parseval, $\norm{Tx}_{\ell^2}=\norm{x}_H$, so $T$ is an isometry. Surjectivity follows from Theorem 10.6, because every sequence in $\ell^2$ produces an element of $H$ with those Fourier coefficients.
\end{proof}

\section{Projection, Direct Sum and Riesz Representation Theorems}

\begin{theoremstmt}{11.1}
\textbf{(Projection theorem)} Let $H$ be a Hilbert space and let $C\subset H$ be nonempty, closed, and convex. Then for every $x\in H$ there exists a unique $y\in C$ such that
\[
\norm{x-y}_H=\inf_{z\in C}\norm{x-z}_H.
\]
If $C=M$ is a closed subspace, then this minimizer is characterized by $x-y\in M^{\perp}$.
\end{theoremstmt}
...
\textbf{(Direct sum theorem)} If $M$ is a closed subspace of a Hilbert space $H$, then
\[
H=M\oplus M^{\perp}.
\]
\end{theoremstmt}
...
\begin{proof}
For each $x\in H$, let $y\in M$ be its projection from Theorem 11.1. Then $x-y\in M^{\perp}$, so $x=y+(x-y)\in M+M^{\perp}$. If $x=y_1+z_1=y_2+z_2$ with $y_i\in M$, $z_i\in M^{\perp}$, then
\[
...
\textbf{(Least squares in $\R^n$)} Let $A\in\R^{m\times n}$ and $b\in\R^m$. A vector $x\in\R^n$ minimizes $\norm{Ax-b}_2$ if and only if it solves the normal equations
\[
A^TAx=A^Tb.
\]
\end{theoremstmt}
...
\textbf{(Riesz representation theorem)} Let $H$ be a Hilbert space. For every $\phi\in H^*$ there exists a unique $a\in H$ such that
\[
\phi(x)=(a,x)_H \qquad\text{for all }x\in H,
\]
and $\norm{\phi}_{H^*}=\norm{a}_H$.
\end{theoremstmt}

\begin{proof}
If $\phi=0$, take $a=0$. Otherwise let $M=\ker\phi$, a closed proper subspace. By Theorem 11.2,
\[
H=M\oplus M^{\perp}.
\]
...
\textbf{(Lax-Milgram lemma, nonsymmetric case)} Let $H$ be a Hilbert space over $\K$, let $B:H\times H\to\K$ be sesquilinear, bounded, and coercive:
\[
\abs{B(u,v)}\le C\norm{u}\norm{v},
\qquad
\operatorname{Re}B(u,u)\ge \beta\norm{u}^2.
\]
...
\]
Thus $A$ satisfies Theorem 12.1, so it is bijective and $\norm{A^{-1}}\le 1/\beta$.
...
If $K\in\mathcal L(H)$ is compact and $x_n\rightharpoonup x$ weakly in $H$, then $Kx_n\to Kx$ strongly in $H$.
\end{theoremstmt}

\begin{proof}
This is Theorem 9.3 specialized to Hilbert spaces.
\end{proof}

\begin{theoremstmt}{13.3}
\textbf{(Fredholm alternative)} Let $H$ be a Hilbert space and let $K\in\mathcal L(H)$ be compact. Then the equation
\[
(I-K)u=f
\]
is solvable if and only if
\[
(f,v)_H=0 \qquad\text{for all }v\in\ker(I-K^*).
\]
Moreover either $\ker(I-K)=\{0\}$ and the solution exists uniquely for every $f$, or $\ker(I-K)$ is nontrivial.
\end{theoremstmt}
...
\textbf{(Neumann series)} Let $T\in\mathcal L(X)$ with $\norm{T}<1$. Then $I-T$ is invertible and
\[
...
\]
is open, and the resolvent map $\lambda\mapsto R(\lambda,T)=(\lambda I-T)^{-1}$ is analytic on $\rho(T)$.
\end{theoremstmt}
...
\[
T^j=\frac{1}{2\pi i}\int_{\Gamma}\lambda^jR(\lambda,T)\,d\lambda.
\]
\end{theoremstmt}
...
If $K\in\mathcal L(H)$ is compact on a Hilbert space $H$, then every nonzero point of $\sigma(K)$ is an eigenvalue. The set $\sigma(K)\setminus\{0\}$ is at most countable, each nonzero eigenvalue has finite multiplicity, and its only possible accumulation point is $0$.
\end{theoremstmt}
...
\textbf{(Hellinger-Toeplitz theorem)} Every everywhere-defined symmetric linear operator on a Hilbert space is bounded.
\end{theoremstmt}
...
\textbf{(Hilbert-Schmidt theorem)} Let $T\in\mathcal L(H)$ be compact and selfadjoint. Then there exists an orthonormal basis $(e_n)$ of $H$ consisting of eigenvectors of $T$, with real eigenvalues $\lambda_n\to 0$, such that
\[
