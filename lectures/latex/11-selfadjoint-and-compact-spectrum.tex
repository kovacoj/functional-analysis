...
Reproduce the calculation: $\lambda(x,y)_H=(Tx,y)_H=(x,Ty)_H=\mu(x,y)_H$, so $(\lambda-\mu)(x,y)_H=0$.
\end{prooflevelbox}

\paragraph{Reason.}
Since $T=T^*$,
\[
\lambda(x,y)_H=(Tx,y)_H=(x,Ty)_H=\mu(x,y)_H.
\]
Thus $(\lambda-\mu)(x,y)_H=0$, so $(x,y)_H=0$.
