...
\paragraph{Why.}
Under the convention linear in the first variable, the Riesz map
\[
H\to H^*,
\qquad
a\mapsto (x\mapsto (x,a)_H)
\]
is conjugate-linear for complex scalars (and linear for real scalars). This still gives an isometric identification of $H$ with $H^*$. Applying Riesz once more identifies $H^{**}$ with $H$ up to the same harmless conjugation, so the canonical embedding $J:H\to H^{**}$ is onto.
