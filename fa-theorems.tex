...
Let $X$ be a normed space. The closed unit ball of $X^*$ is compact in the weak-* topology. In particular, if $X$ is separable, every bounded sequence in $X^*$ has a weak-* convergent subsequence.%
}
...
Let $(X,d)$ be complete and let $F_1\supset F_2\supset \cdots$ be nonempty closed sets with $\operatorname{diam}(F_n)\to 0$. Then $\bigcap_{n=1}^{\infty}F_n$ consists of exactly one point.%
}
\newcommand{\thmBaireCategory}{%
A complete metric space cannot be a countable union of nowhere dense sets. Equivalently, a countable intersection of dense open sets is dense.%
}
\newcommand{\thmBaireVariant}{%
If a complete metric space is the union of countably many closed sets, then at least one of those closed sets has nonempty interior.%
}
...
Let $X,Y$ be Banach spaces and let $L\in\mathcal L(X,Y)$ be surjective. Then $L$ maps open sets in $X$ onto open sets in $Y$.%
}
\newcommand{\thmBoundedInverse}{%
Let $X,Y$ be Banach spaces and let $L\in\mathcal L(X,Y)$ be bijective. Then $L^{-1}\in\mathcal L(Y,X)$.%
}
...
A bounded linear operator $K:X\to Y$ is compact if and only if its adjoint $K^*:Y^*\to X^*$ is compact.%
}
...
Let $H$ be a Hilbert space and let $K\in\mathcal L(H)$ be compact. Then the equation
$(I-K)u=f$
...
Every everywhere-defined symmetric linear operator on a Hilbert space is bounded.%
}
...
$Tx=\sum_{n=1}^{\infty}\lambda_n(x,e_n)e_n \qquad\text{for all }x\in H.$%
}
