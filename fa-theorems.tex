...
Let $H$ be a Hilbert space over $\K$, let $B:H\times H\to\K$ be sesquilinear, bounded, and coercive, linear in the first argument and conjugate-linear in the second:
...
and $\norm{u}\le \beta^{-1}\norm{f}$.
In the real case the convention issue disappears; in the complex case the order of the variables matters.\label{thm:lax-milgram-general}%
}
...
\item $\operatorname{Ran}(I-K)$ is closed and
$\operatorname{Ran}(I-K)=\ker(I-K^*)^{\perp}$;
\item $\dim\ker(I-K)=\dim\ker(I-K^*)$;
\item the equation $(I-K)u=f$ is solvable if and only if
$(f,v)_H=0$ for every $v\in\ker(I-K^*)$;
